<p align="center">
  <img src="AGI -Artificial General Intelligence.png" width="100%" />
</p>

# 🚀 Benoît | Étudiant

<p align="left">
  <img src="https://img.shields.io/badge/ÉCOLE-HOLBERTON_SCHOOL_TOULOUSE-red?style=for-the-badge" />
 </p>

### 👨‍💻 À propos
Je suis actuellement **étudiant à Holberton School Toulouse**. Je prépare un **titre RNCP 5 (Développeur Web et Web Mobile)**.
J'alimenterais mon Github tout au long de l'année avec mes projets scolaires et persos!

Mon parcours est guidé par la curiosité et l'efficacité :
- 🎓 **Formation :** Holberton School Toulouse (Focus actuel : Shell, Langage C & Algorithmique).
- 🎯 **Objectif :** Me former en Cybersécurité, l'intelligence artificielle et Data Analyse.
- 🗣 **Particularité :** J'ai d'autres expériences, je suis ingénieur généraliste industriel diplômé de l'EIGSI La Rochelle en 2007, je suis sorti de Math sup/Math spé après une 5/2 en 2004. J'effectue une reconversion en développement informatique avec plusieurs fils conducteurs : La compréhension et l'optimisation des systèmes, l'analyse et l'exploitation de données, la réglementation d'entreprise et ses processus, l'analyse des risques HACCP et leur correction, définir une stratégie de communication. L'informatique est pour moi une manière d'obtenir la rigueur et l'esprit d'analyse nécessaire à tous les postes de l'entreprise.

---

### 🛠 Compétences Techniques
- **Langages :** C (en cours), Shell & Bash (maîtrisés).
- **Automation & Outils :Git, GitHub.
- **En préparation :** Titre RNCP 5 - Développeur Web & Web Mobile.
<p align="left">
  <img src="https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/Shell-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
</p>
---

### 🧠 Soft Skills & Passions
- **Empathe :** Capacité d'écoute, d'enquête et d'évaluation professionnelle
- ** :** 
- **Peer-Learning :** Apprentissage collaboratif et autonomie renforcée par la méthode Holberton.
- 
---

### 📫 Me contacter
- **LinkedIn :** [https://www.linkedin.com/in//]
- **Email :** [13048@holbertonstudents.com]
